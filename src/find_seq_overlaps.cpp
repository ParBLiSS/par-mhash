...
//
template<typename KMER, typename HBT, std::size_t seed_begin>
struct MinHashFunctionBlock {

    using value_type = HBT;

    HBT operator()(const KMER& tx){
...
template<std::size_t seed_index>
using SeqMinHashGeneratorType = SeqMinHashGenerator<
    MinHashFunctionBlock<KmerType, HashBlockType, seed_index>, KmerType, seed_index>;

template<std::size_t seed_index>
using RHBType = typename SeqMinHashGeneratorType<seed_index>::value_type;
...
void generateSequencePairs(mxx::comm& comm,
  ...
  std::vector<RHBType<seed_index>>  local_rhpairs;

  bliss::io::KmerFileHelper::template
    parse_file_data<SeqMinHashGeneratorType<seed_index>, FileParser,
  ...
  mxx::sort(local_rhpairs.begin(), local_rhpairs.end(),
            [&] (const RHBType<seed_index>& x, const RHBType<seed_index>& y){
...
void runFSO(mxx::comm& comm,
  ...
  BL_BENCH_START(rfso);
  generateSequencePairs<0>(comm, file_data.back());
  generateSequencePairs<1>(comm, file_data.back());
